...
def national_totals(district_rows):
    ...
    nationals.update({
            key: sum(map(lambda i: int(i[key]), group)) for key in eggs
    })
    return [nationals]
...
def eggs_for_district(eggs_row_tuple):
    ...
    district.update({
        k: getattr(eggs_row_tuple, k) for k in eggs
    })
    return [district]
...
def convert_csv(inputfile, outputfile):
    ...
    with open(inputfile, 'r') as data, open(outputfile, 'w') as csv_out:
        ...
        district_data = [row for district_eggs
                         in
                         [eggs_for_district(
                             ConvertedRow(
