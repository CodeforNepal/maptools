...
MaternalDeathRow = namedtuple('MaternalDeathRow', ['district_code',
                                                   'district_name',
                                                   'antepartum',
                                                   'intrapartum',
                                                   'postpartum'])
...
def convert_csv(inputfile, outputfile):
    ...
    with open(inputfile, 'r') as data, open(outputfile, 'w') as csv_out:
        ...
        district_data = [row for district_maternal_deaths
                         in
                         [deaths_for_district(
                             MaternalDeathRow(row[COLUMNS['district_code']],
        ...
        national_row = [MaternalDeathRow(row[COLUMNS['district_code']],
                                              row[COLUMNS['district_name']],
                                              row[COLUMNS['antepartum']],
                                              row[COLUMNS['intrapartum']],
                                              row[COLUMNS['postpartum']])
                        for row in csv_rows
                        if row[COLUMNS['district_name']] == 'National Total'][0]
        national_data = [
        {
            'geo_code': 'NP',
            'geo_level': 'country',
        ...
        for row in national_data + district_data:
            writer.writerow([row['geo_level'], row['geo_code'],
                             row['maternal_death'], row['total']])
...
def main(args):
    ...
    try:
        ...
        print('python maternaldeaths.py '
              '-i <inputfile> '
              '-o <outputfile> ')
        sys.exit(2)
    for opt, arg in opts:
