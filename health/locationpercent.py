    ...
    return {
        ...
        stat_name:
            "{value:.{digits}f}".format(
                value=float(location_percent_tuple.percent),
                digits=2)
            if location_percent_tuple.percent else 0.0
    }
...
def convert_csv(inputfile, outputfile, column_index, stat_name):
    ...
    with open(inputfile, 'r') as data, open(outputfile, 'w') as csv_out:
        ...
        parties_data = [stat_dict(LocationPercent(row[1],
                                                  row[column_index]),
                                  stat_name)
                        for row in csv_rows
...
def main(args):
    ...
    stat_name = ''
    try:
        opts, args = getopt.getopt(args, 'hi:o:c:s:',
                                   ['inputfile=',
                                    'outputfile=',
                                    'columnindex=',
                                    'statname='])
    except getopt.GetoptError:
        print('python locationpercent.py '
