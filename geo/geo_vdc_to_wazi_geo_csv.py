...
def vdc_names(infilename, outfilename):
    ...
    with open(infilename, 'r') as infile, open(outfilename, 'w') as outfile:
        ...
        fieldnames = ['name', 'geo_code', 'year', 'parent_level',
                      'long_name', 'geo_level', 'parent_code']
        writer = csv.DictWriter(outfile,
                                fieldnames=fieldnames)
        writer.writeheader()
        districtgetter = itemgetter('parent_code')
        district_groups = []
        for key, group in groupby(sorted(vdcs, key=districtgetter),
                                  districtgetter):
            district_groups.append(sorted(list(group), key=lambda
                vdc: vdc['geo_code']))
        writer.writerows([vdc for district_vdcs in district_groups for vdc in
                          district_vdcs])
