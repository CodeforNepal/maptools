...
def voter_totals(parties_tuple):
    ...
    return {
        ...
        'geo_code': geo_code,
        'registered_parties':
            int(parties_tuple.parties) if parties_tuple.parties else 0
    }
...
def convert_csv(inputfile, outputfile):
    ...
    with open(inputfile, 'r') as data, open(outputfile, 'w') as csv_out:
        ...
        polling_place_data = [voter_totals(Parties(*row)) for row in csv_rows]
