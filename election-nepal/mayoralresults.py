...
ElectionResults = namedtuple('ElectionResults',
                             ['district', 'localbodies', 'cpn',
                              'congress', 'maoist',
                              'rpp', 'others'])
...
def results_for_district(results_tuple):
    ...
    party_results = [
        ...
        {'geo_level': 'district',
         ...
         'total': int(results_tuple.congress)
         if results_tuple.congress else 0
         },
        {'geo_level': 'district',
         'geo_code': geo_code,
         'party': 'Maoist Kendra',
         'total': int(results_tuple.maoist)
         if results_tuple.maoist else 0
         },
        {'geo_level': 'district',
         'geo_code': geo_code,
        ...
        {'geo_level': 'district',
         'geo_code': geo_code,
         'party': 'Others',
         'total': int(results_tuple.others)
...
def convert_csv(inputfile, mayoraloutput, deputyoutput):
    print('input: {},\nmayoralout: {},\ndeputyout:{}'.format(inputfile,
                                                               mayoraloutput,
                                                               deputyoutput))
    with open(inputfile, 'r') as data, open(mayoraloutput, 'w') as mayor_out, \
            open(deputyoutput, 'w') as deputy_out:
        ...
        district_deputy_mayoral_data = [district_item for district_list in
                                        [results_for_district(ElectionResults(
                                            ...
                                            for row in csv_rows]
                                        for district_item in district_list]
        all_mayoral = national_totals(district_mayoral_data) + \
            district_mayoral_data
        all_deputy_mayoral = national_totals(district_deputy_mayoral_data) + \
            district_deputy_mayoral_data
        mayor_writer = csv.writer(mayor_out)
        mayor_writer.writerow(csv_keys)
        for row in all_mayoral:
            mayor_writer.writerow([row['geo_level'],
                                   row['geo_code'],
                                   row['party'],
                                   row['total']])
        ...
        for row in all_deputy_mayoral:
            deputy_writer.writerow([row['geo_level'],
                                    row['geo_code'],
                                    row['party'],
                                    row['total']])
...
def main(args):
    ...
    deputyoutput = ''
    try:
        opts, args = getopt.getopt(args, 'hi:m:d:',
                                   ['inputfile=',
                                    'mayoraloutput=',
                                    'deputyoutput='])
    except getopt.GetoptError:
        print('python localbodies.py '
              '-i <inputfile> '
              '-m <mayoraloutput>  '
              '-d <deputyoutput> ')
        sys.exit(2)
