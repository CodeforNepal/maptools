...
def national_totals(rows, headertext, columnname):
    ...
    national_list = []
    for key, group in groupby(sorted(rows, key=variable_getter),
                              variable_getter):
        national_list.append({
            ...
            columnname: key,
            'total': sum(map(
                lambda item: int(item['Value']) if item['Value'] else 0,
                group))
        })
...
def add_missing_values_for_area(all_keys, tuples_for_area):
    variables_not_in_area = set(all_keys) - set(map(lambda tup: tup[0],
                                                    tuples_for_area))
...
def district_totals(rows, headertext, columnname):
    ...
    districts_dict = {}
    for key, group in groupby(sorted(rows, key=district_getter),
                              district_getter):
        district = key[0]
        variable = key[1]
    ...
    for district in districts_dict:
        totals_for_district = sorted(districts_dict[district].items(),
                                     key=itemgetter(1), reverse=True)
        not_in_district = add_missing_values_for_area(
            all_keys, totals_for_district)
        district_data.append((district, totals_for_district +
                              not_in_district))

    return [item for sublist in
            list(map(rows_for_district(columnname), district_data)) for
            item in sublist]
...
def convert_csv(inputfile, outputfile, headertext, columnname):
    ...
    with open(inputfile, 'r') as data:
        ...
        all_data = sorted(national_data + district_data,
                          key=lambda r: '{}{}{}'.
...
def main(args):
    ...
    columnname = ''
    try:
        opts, args = getopt.getopt(args, 'hi:o:t:c:',
                                   ['inputfile=',
                                    'outputfile=',
                                    'headertext=',
                                    'columnname='])
