...
def all_years_including_zeros(location_years_with_data, company_types):
    ...
    for company_type in company_types:
        years_for_type = [row['year'] for row in location_years_with_data
...
def convert_csv(input_file, output_file):
    ...
    with open(input_file, 'r') as data:
        ...
        all_data = [row for row in reader
                         if row['DISTRICT'] != 'District not known']
        ...
        for district, district_group in groupby(
            ...
            for company_type, company_group in groupby(
                    sorted(list(district_group),
                           key=company_type_getter),
                    company_type_getter):
                year_rows_for_type = []
                for year, year_group in groupby(sorted(list(company_group),
                                                       key=year_getter),
                                                year_getter):
...
def main(args):
    ...
    for opt, arg in opts:
        if opt == '-h':
            print('python registeredcompanies.py -i <inputcsv> '
                  '-o <outputcsv>')
            sys.exit()
